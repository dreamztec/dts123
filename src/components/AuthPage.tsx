...
export function AuthPage({ mode }: { mode:'login'|'register'|'forgot' }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (mode === 'register') { const user = await signup(email,password,{ full_name:name, role:'customer' }); setStatus(user.confirmedAt ? 'Account created. Opening your mobility workspace…' : 'Account created. Check your email to verify your address.') }
