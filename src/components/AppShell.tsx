...
export function AppShell({ kind, title, subtitle, children }: { kind: ShellKind; title: string; subtitle: string; children: React.ReactNode }) {
  ...
  </aside><section className="workspace"><header className="workspace-header"><div><h1>{title}</h1><p>{subtitle}</p></div><div className="workspace-actions"><button className="icon-button" aria-label="Open menu"><Menu size={17}/></button><button className="icon-button" aria-label="Notifications"><Bell size={17}/></button><div className="profile-chip"><span className="avatar">FR</span><span>FASTRIDES workspace</span></div></div></header><div className="workspace-main">{children}</div></section></div>
}